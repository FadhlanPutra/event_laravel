            ...
            <div class="bg-white relative shadow-md sm:rounded-lg overflow-hidden">
                <div class="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 p-4">
                    <div class="w-full md:w-1/2">
                        <form class="flex items-center" action="{{ route('event') }}" method="GET">
                            ...
                            <div class="relative w-full">
                                ...
                                <input value="{{ $searchTerm }}" type="text" id="search" name="search" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full pl-10 p-2 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Search" autocomplete="off">
                            </div>
                    ...
                    <table class="w-full text-sm text-left text-gray-500">
                        ...
                        <tbody>
                            ...
                            <tr class="border-b">
                                ...
                                <td class="px-4 py-3">{{ $event->nama_event }}</td>
                                <td class="px-4 py-3">{{ date('l, M-d-Y', strtotime($event->tanggal)) }}</td>
                                <td class="px-4 py-3">{{ $event->lokasi }}</td>
                                <td class="px-4 py-3">
                                    <img class="mx-auto h-full object-cover min-w-24 lg:w-[75%] max-h-48 lg:max-h-56 " src="{{ asset('storage/' . $event->foto) }}" alt="{{ $event->foto }}">
                                </td>
                                <td class="px-4 py-3">{{ $event->deskripsi }}</td>
                                <td class="px-4 py-3">
                                    <a href="{{ route('event.show', $event->id) }}" class="bg-gray-600 p-2 rounded-lg text-white">Show</a>
                                    <button type="button" data-id="{{ $event->id }}" class="bg-amber-600 p-2 m-2 rounded-lg text-white" data-bs-target="#editModal{{ $event->id }}" id="editModalButton" data-modal-target="editModal{{ $event->id }}" data-modal-toggle="editModal{{ $event->id }}">Edit</button>
                                    @method('DELETE')
                                    <a style="cursor: pointer;" onclick="event.preventDefault(); if(confirm('Yakin ingin menghapus data ini?')) document.getElementById('delete-form-{{ $event->id }}').submit();" class="bg-red-600 p-2 rounded-lg text-white">Hapus</a>
                                </td>
                            ...
                            <!-- EDIT MODAL -->
<div id="editModal{{ $event->id }}" aria-labelledby="editModalLabel{{ $event->id }}" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-modal md:h-full">
    <div class="relative p-4 w-full max-w-2xl h-full md:h-auto">
        ...
        <div class="relative p-4 bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
            ...
            <div class="flex justify-between items-center pb-4 mb-4 rounded-t border-b sm:mb-5 dark:border-gray-600">
                ...
                <button type="button" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-toggle="editModal{{ $event->id }}">
                    <svg aria-hidden="true" class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd"></path></svg>
                    <span class="sr-only">Close modal</span>
                </button>
            </div>
            <!-- Modal body -->
            <form action="{{ route('event.update', $event->id) }}" id="editForm{{ $event->id }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
